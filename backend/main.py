...
def _resolve_confirm_intent(intent: str, extracted_data: dict | None) -> str:
    ...
    if normalized in {"insert", "update", "drop_table", "alter_table", "delete_data"}:
        return normalized
    return ""
...
def _execute_operation(operation_intent: str, extracted_data: dict) -> tuple[str, dict | None, str | None]:
    ...
    if intent in {"insert", "row_insert"}:
        table = _normalize_identifier((extracted_data.get("table") or "").strip(), "表名")
        data = _extract_data_from_payload(payload) if payload is not None else extracted_data.get("data", {})
        ...
        return f"✅ 数据已插入表「{table}」，ID: {result.get('rowid')}", patch, intent

    if intent in {"update", "row_update"}:
        table = _normalize_identifier((extracted_data.get("table") or "").strip(), "表名")
        data = _extract_data_from_payload(payload) if payload is not None else extracted_data.get("data", {})
        where = _extract_where_from_payload(payload) if payload is not None else extracted_data.get("where", {})
        if not where and isinstance(extracted_data.get("where"), dict):
            where = extracted_data.get("where", {})
        if not where:
            raise HTTPException(status_code=400, detail="更新操作缺少行定位条件")
        ...
        patch = {"type": "rows", "table": table, "refresh": True}
        return f"✅ 已更新表「{table}」，影响行数: {result.get('rows_affected')}", patch, intent
    ...
    if intent in {"delete_data", "row_delete"}:
        ...
        if where:
            ...
            patch = {"type": "rows", "table": table, "refresh": True}
            return f"✅ 已从表「{table}」删除 {result.get('rows_affected')} 条记录", patch, intent

        where_clause = extracted_data.get("where_clause", "")
        ...
        try:
            ...
            conn.close()
        patch = {"type": "rows", "table": table, "refresh": True}
        return f"✅ 已从表「{table}」删除 {rows} 条记录", patch, intent
    ...
    if intent == "drop_table":
        ...
        return f"✅ 已删除表「{table}」", patch, intent
    ...
    if intent in {"alter_table", "add_col", "drop_col", "rename_col"}:
        ...
        if intent == "add_col":
            col = _normalize_identifier((payload or {}).get("column") or "", "字段名")
            col_type = ((payload or {}).get("column_type") or "TEXT").strip().upper()
            add_column(table, col, col_type, bool((payload or {}).get("notnull", False)), str((payload or {}).get("default", "")))
            patch = {"type": "schema", "table": table, "refresh": True}
            return f"✅ 已在表「{table}」新增字段「{col}」", patch, intent

        if intent == "drop_col":
            col = _normalize_identifier((payload or {}).get("column") or "", "字段名")
            drop_column(table, col)
            patch = {"type": "schema", "table": table, "refresh": True}
            return f"✅ 已从表「{table}」删除字段「{col}」", patch, intent

        if intent == "rename_col":
            old = _normalize_identifier((payload or {}).get("column") or "", "原字段名")
            new = _normalize_identifier((payload or {}).get("new_name") or "", "新字段名")
            rename_column(table, old, new)
            patch = {"type": "schema", "table": table, "refresh": True}
            return f"✅ 已将表「{table}」字段「{old}」重命名为「{new}」", patch, intent

        # legacy alter_table 路径
        ...
        try:
            ...
            conn.close()
        patch = {"type": "schema", "table": table, "refresh": True}
        return f"✅ 表结构修改完成：{extracted_data.get('description', '')}", patch, intent

    if intent == "cell_get":
        table = _normalize_identifier((extracted_data.get("table") or "").strip(), "表名")
        where = _extract_where_from_payload(payload)
        column = _normalize_identifier((payload or {}).get("column") or "", "字段名")
        _validate_table_and_columns(table, list(where.keys()) + [column])
        result = DBCellGetTool().run(table=table, where=where, column=column)
        patch = {"type": "rows", "table": table, "refresh": False}
        message = f"✅ 单元格查询结果：表「{table}」中 {where} 的「{column}」= {result.get('value')}"
        return message, patch, intent
    ...
    if intent == "cell_update":
        ...
        _validate_table_and_columns(table, list(where.keys()) + [column])
        DBCellUpdateTool().run(table=table, where=where, column=column, value=value)
        patch = {"type": "rows", "table": table, "refresh": True}
        return f"✅ 已更新表「{table}」中 {where} 的「{column}」", patch, intent
